    ...
    <div id='nav' className="bg-[url('../public/Footer.png')] h-28 flex">
      <Container maxWidth="lg" className='relative flex flex-col gap-4 max-sm:w-[400px]'>
        ...
        <Stack direction="row" className="w-[100%] justify-between">
          <Stack direction="row" className="items-end justify-between  transition-all text-white mx-3 gap-3 h-20  w-[50%] ">
  ...
  <Link key={index} href={`/${item == "Haqqımızda" ? "About" : item == "Kataloq" ? "Catalog" : item == "Yeniliklər" ? "News" : item == "Çatdırılma" ? "Delivery" :  item == "Əlaqə" ? "Contact" : ""}`}>
    <Typography variant="body2" className="group transition-all duration-200 bg-clip-text block cursor-pointer mont max-sm:hidden hover:bg-gradient-to-r from-yellow-700 to-gradient2 hover:text-transparent hover:underline">
      {item}
          ...
          <Stack direction="row" className="transition-all items-start  justify-between w-[30%] max-sm:w-[100%]">
